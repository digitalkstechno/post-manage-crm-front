...
export default function LoginView({ onLogin }: LoginViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    // await onLogin(email, password, null);
    await onLogin(email, password, 'staff');
    setIsLoading(false);
  };
        ...
        <div className="bg-white/70 backdrop-blur-lg rounded-[2.5rem] border border-white p-10 shadow-2xl shadow-slate-200/50">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-primary text-white font-black py-5 rounded-2xl flex items-center justify-center gap-3 hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 shadow-2xl shadow-primary/30 uppercase tracking-widest text-xs"
            >
              {isLoading ? (
