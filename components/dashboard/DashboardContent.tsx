...
interface DashboardContentProps {
  ...
  addSubmission: (s: Omit<Submission, 'id' | 'status' | 'createdAt' | 'staffName' | 'staffEmail'>) => void;
  updateStatus: (id: string, status: SubmissionStatus, comment?: string) => void;
}

export default function DashboardContent({ role, activeTab, submissions, addSubmission, updateStatus }: DashboardContentProps) {
    ...
    <div className="p-8 pb-20 max-w-7xl mx-auto space-y-8">
      ...
      {(activeTab === 'overview' || activeTab === 'submissions') && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <StatCard label="Total Submissions" value={stats.total} icon={FileText} trend="+12% ↑" color="blue" />
          <StatCard label="Pending Requests" value={stats.pending} icon={Clock} trend="Action Req" color="amber" />
          <StatCard label="Total Approved" value={stats.approved} icon={CheckCircle2} trend="93% Rate" color="emerald" />
          <StatCard label="Total Rejected" value={stats.rejected} icon={XCircle} trend="Manual Rev." color="rose" />
        </div>
      )}
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200 p-6 h-64 flex flex-col justify-between">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Submission Trends</h4>
                <div className="flex items-end justify-between h-32 gap-3 px-2">
                  {[40, 60, 45, 80, 95, 70, 90].map((h, i) => (
                    <div key={i} className="flex-1 bg-slate-100/50 rounded-t-xl relative group transition-all hover:bg-primary/5">
                      <div className="absolute inset-x-1 bottom-0 bg-primary/20 rounded-t-lg scale-y-0 origin-bottom transition-transform duration-700" style={{ height: `${h}%`, transform: 'scaleY(1)' }} />
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-primary rounded-2xl p-8 text-white relative overflow-hidden flex flex-col justify-between shadow-2xl shadow-primary/30">
                <div className="relative z-10">
                  <h4 className="text-[10px] font-black opacity-60 uppercase tracking-[0.2em] mb-2">System Health</h4>
                  <p className="text-xl font-extrabold tracking-tight">Active & Secure</p>
                </div>
          ...
          >
            <div className="flex-1 bg-white rounded-3xl border border-slate-200 p-10 shadow-sm">
              <div className="flex items-center gap-3 mb-8 pb-4 border-b border-slate-50">
                ...
                <h3 className="text-xl font-bold text-slate-800 tracking-tight">Submit New Documentation</h3>
              </div>
              
              <form onSubmit={handleUploadSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="col-span-2 space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-slate-400 ml-1">Document Title</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-slate-400 ml-1">Drive / File Link</label>
                    <input 
                      type="url" 
                      value={formData.link}
                  ...
                  <div className="col-span-2 space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-slate-400 ml-1">Context / Description</label>
                    <textarea 
            ...
            <div className="w-full md:w-80 space-y-6">
              <div className="bg-primary rounded-3xl p-8 text-white relative overflow-hidden shadow-xl shadow-primary/10">
                 <h4 className="text-sm font-bold opacity-60 uppercase tracking-widest mb-2">Submission Policy</h4>
                 <p className="text-sm leading-relaxed opacity-90 mb-6">
                   Ensure all documents are properly categorized and links are shared for external viewing to avoid rejection.
                 </p>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[60] flex items-center justify-center p-6">
          ...
          >
            ...
            <p className="text-slate-500 text-sm mb-6">
              Provide a reason for the rejection. This feedback will be visible to the staff member.
            </p>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-slate-50 border-b border-slate-100">
            ...
            {role === 'admin' && <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Staff</th>}
                ...
                <td className="px-6 py-4">
                  <span className={cn(
                    ...
                    <div className={cn("w-1.5 h-1.5 rounded-full", 
                      s.status === 'approved' ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : 
                      s.status === 'rejected' ? "bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]" : 
                      ...
                      <>
                        ...
                        {s.adminComment && (
                          <div className="relative group/comment">
                            <MessageSquare size={16} className="text-amber-400 cursor-help" />
      ...
      <div className="p-6 bg-slate-50/30 border-t border-slate-100 flex justify-between items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
         ...
         <div className="flex gap-2">
            <button className="px-3 py-1 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-30" disabled>Prev</button>
            <button className="px-3 py-1 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-30" disabled>Next</button>
         </div>
      </div>
    </div>
  );
